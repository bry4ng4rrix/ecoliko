...
import {
  ...
  fetchEtudiantCodeBarreUrl,
  fetchEtudiantQrCodeUrl,
  fraisScolariteService,
  genererCertificatScolarite,
  inscriptionService,
  paiementService,
...
export function EtudiantsPanel() {
  ...
  const { data: etudiants, isLoading } = useResourceList(
    "etudiants",
    etudiantService,
  );
  ...
  const createInscription = useCreateResource(
    "inscriptions",
    inscriptionService,
  );
  ...
  const startEdit = (etudiant) => {
    ...
    setForm({
      ...
      ancien_etablissement: etudiant.ancien_etablissement ?? "",
      dossier_medical: etudiant.dossier_medical ?? "",
      contact_urgence_nom: etudiant.contact_urgence_nom ?? "",
      contact_urgence_telephone: etudiant.contact_urgence_telephone ?? "",
    });
    setDialogOpen(true);
  };
    ...
    () => [
      {
        ...
        header: "Matricule",
        cell: ({ row }) => (
          <span className="font-mono">{row.original.matricule}</span>
        ),
      },
      {
        id: "nom",
        header: "Nom",
        accessorFn: (s) => `${s.prenom} ${s.nom}`,
        cell: ({ row }) => (
      ...
      },
      {
        ...
        header: "Statut",
        cell: ({ row }) => (
          <Badge variant="secondary">{row.original.statut}</Badge>
        ),
      },
      {
        id: "actions",
        ...
        cell: ({ row }) => (
          <div className="text-center space-x-2">
            <button
              ...
              onClick={() => setDossierEtudiant(row.original)}
              title="Dossier"
            >
            ...
            <button
              className="text-primary hover:underline"
              onClick={() => setFinanceEtudiant(row.original)}
              title="Paiements"
            >
            ...
            <button
              className="text-primary hover:underline"
              onClick={() => setClasseEtudiant(row.original)}
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Inscription, modification et suivi des profils
          </p>
        </div>
        <Dialog
          open={dialogOpen}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <Input
                    ...
                    className={editing ? "text-muted-foreground" : ""}
                  />
                  <Input
                    name="prenom"
                    placeholder="Prénom"
                    ...
                    >
                      ...
                      {(classes ?? []).map((c) => (
                        <option key={c.id} value={c.id}>
                          {c.nom}
                          {c.filiere_intitule ? ` — ${c.filiere_intitule}` : ""}
                        </option>
                      ))}
                    </select>
                  )}
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <Input
                    ...
                    placeholder="Ancien établissement (facultatif)"
                    value={form.ancien_etablissement}
                    onChange={handleChange}
                  />
                </div>
                <textarea
                  name="dossier_medical"
                  placeholder="Dossier médical (allergies, traitements... facultatif)"
                  value={form.dossier_medical}
                  onChange={handleChange}
                  rows={2}
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  className="flex-1"
                  disabled={
                    createEtudiant.isPending || updateEtudiant.isPending
                  }
                >
      ...
      <DataTable
        ...
        data={
          filtreClasse
            ? (etudiants ?? []).filter(
                (e) => e.classe_actuelle === filtreClasse,
              )
            : (etudiants ?? [])
        }
        isLoading={isLoading}
      ...
      {financeEtudiant && (
        <PaiementsEtudiantDialog
          ...
          onClose={() => setFinanceEtudiant(null)}
        />
      )}
      {classeEtudiant && (
        <ChangerClasseDialog
          etudiant={classeEtudiant}
          onClose={() => setClasseEtudiant(null)}
        />
      )}
    </div>
  );
...
function ChangerClasseDialog({ etudiant, onClose }) {
  ...
  const { data: classes } = useResourceList("classes", classeService);
  const { data: inscriptions, isLoading } = useResourceList(
    "inscriptions",
    inscriptionService,
  );
  const createInscription = useCreateResource(
  ...
  const updateInscription = useUpdateResource(
    "inscriptions",
    inscriptionService,
  );
  ...
  useEffect(() => {
    if (inscriptionActive && !classeId)
      setClasseId(String(inscriptionActive.classe));
    // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    try {
      if (inscriptionActive) {
        await updateInscription.mutateAsync({
          ...
          payload: { classe: Number(classeId) },
        });
      } else {
        await createInscription.mutateAsync({
          etudiant: etudiant.id,
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Changer de classe — {etudiant.prenom} {etudiant.nom}
          </DialogTitle>
        </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <p className="text-sm text-muted-foreground">
              ...
              <span className="font-medium text-foreground">
                {etudiant.classe_actuelle ?? "Aucune"}
              </span>
            </p>
            <select
              value={classeId}
...
function PaiementsEtudiantDialog({ etudiant, onClose }) {
  ...
  const { data: fraisScolarite } = useResourceList(
    "frais-scolarite",
    fraisScolariteService,
  );
  const { data: classes } = useResourceList("classes", classeService);
  const { data: inscriptions } = useResourceList(
    "inscriptions",
    inscriptionService,
  );
  const { data: anneesScolaires } = useResourceList(
    "annees-scolaires",
    anneeScolaireService,
  );
  ...
  const tarifNiveau = classeActuelle
    ? (fraisScolarite ?? []).find(
        (f) =>
          f.annee_scolaire === anneeActive?.id &&
  ...
  const estReinscription =
    ...
    (inscriptions ?? []).some((i) => {
      ...
      const anneeDeLInscription = (anneesScolaires ?? []).find(
        (a) => a.id === i.annee_scolaire,
      );
      return (
        anneeDeLInscription &&
        anneeDeLInscription.date_debut < anneeActive.date_debut
      );
    });
  ...
  const montantInscription =
    droitClasse ?? tarifNiveau?.montant_inscription ?? null;
  ...
  const moisEnRetard = anneeActive
    ? Array.from({ length: 12 }, (_, i) => i + 1).filter((mois) => {
        const dejaPaye = paiementsParMois(mois).some(
          (p) => p.statut === "PAYE",
        );
  ...
  const handleGenererCarteEcolage = async () => {
    try {
      await telechargerCarteEcolage(
        etudiant.id,
        `carte_ecolage_${etudiant.matricule}.pdf`,
      );
    } catch (err) {
      const data = err.response?.data;
  ...
  const handleGenererFactureInscription = async () => {
    try {
      await telechargerFactureEcolage(
        etudiant.id,
        { anneeScolaireId: anneeActive.id, inscription: true },
        `facture_inscription_${etudiant.matricule}.pdf`,
      );
    } catch (err) {
      const data = err.response?.data;
      toast.error(
        data?.detail ?? "Erreur lors de la génération de la facture.",
      );
    }
  };
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[85vh] overflow-y-auto max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            Paiements — {etudiant.prenom} {etudiant.nom}
          </DialogTitle>
        </DialogHeader>
        ...
        <Button
          ...
          onClick={handleGenererCarteEcolage}
        >
        ...
        {!anneeActive ? (
          ...
          <div className="space-y-6">
            {(loadingDossier || loadingPaiements) && (
              <p className="text-sm text-muted-foreground">Chargement...</p>
            )}
            ...
            {dossier && (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                ...
                <div className="bg-muted rounded-lg p-3">
                  ...
                  <Badge
                    variant={
                      dossier.statut === "PAYE" ? "default" : "secondary"
                    }
                    className="mt-1"
                  >
                    {dossier.statut}
                ...
                <div className="bg-muted rounded-lg p-3">
                  ...
                  <p
                    className={`text-lg font-bold ${moisEnRetard > 0 ? "text-red-600" : ""}`}
                  >
                    {moisEnRetard}
                  </p>
                </div>
            ...
            <div>
              ...
              {montantInscription == null && montantEcolageMensuel == null ? (
                <p className="text-sm text-muted-foreground">
                  Aucun tarif configuré (ni sur la classe, ni par niveau) pour
                  cette année.
                </p>
              ) : (
                <div className="flex flex-wrap gap-3">
                  <div className="flex-1 min-w-[200px] bg-muted rounded-lg px-3 py-2 flex justify-between items-center">
                    ...
                    <div className="ml-3 flex items-center gap-2">
                      <button
                        ...
                        className="text-xs px-2 py-1 bg-blue-500/20 text-blue-700 rounded hover:bg-blue-500/30 font-medium"
                      >
                        Générer facture
                      </button>
                      <button
                  ...
                  <div className="flex-1 min-w-[200px] bg-muted rounded-lg px-3 py-2 flex justify-between items-center">
                    ...
                    <span className="text-sm font-mono">
                      {montantEcolageMensuel != null
                        ? `${Number(montantEcolageMensuel).toLocaleString("fr-FR")} Ar/mois`
                        : "—"}
                    </span>
                  </div>
                </div>
            ...
            <div>
              <h3 className="font-semibold text-sm mb-2">
                Cartes d'écolage — suivi mensuel
              </h3>
                        ...
                        return (
                          <tr key={mois}>
                            ...
                            <td className="px-3 py-2">
                              <Badge
                                variant={enRetard ? "destructive" : "secondary"}
                              >
                          ...
                          <td className="px-3 py-2 text-center">
                            {dejaPaye ? (
                              <div className="flex items-center justify-center gap-2">
                                <button
                                  ...
                                  onClick={() =>
                                    telechargerFactureEcolage(
                                      ...
                                      `facture_${etudiant.matricule}_${mois}.pdf`,
                                    )
                                  }
                                  className="text-xs px-2 py-1 bg-blue-500/20 text-blue-700 rounded hover:bg-blue-500/30 font-medium"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleMarquerNonPaye(mois)}
...
function DossierEtudiantDialog({ etudiant, onClose }) {
  ...
  const { data: documents, isLoading: loadingDocs } = useResourceList(
    "documents-etudiants",
    documentEtudiantService,
  );
  const createDocument = useCreateResource(
    "documents-etudiants",
    documentEtudiantService,
  );
  ...
  const historique = (inscriptions ?? []).filter(
    (i) => i.etudiant === etudiant.id,
  );
  const mesDocuments = (documents ?? []).filter(
    (d) => d.etudiant === etudiant.id,
  );

  const voirQrCode = async () =>
    setQrUrl(await fetchEtudiantQrCodeUrl(etudiant.id));
  const voirCodeBarre = async () =>
    setBarcodeUrl(await fetchEtudiantCodeBarreUrl(etudiant.id));
  ...
  const handleGenererCarte = async () => {
    try {
      await telechargerCarteEtudiant(
        etudiant.id,
        `carte_${etudiant.matricule}.pdf`,
      );
    } catch {
  ...
  const handleGenererCertificat = async () => {
    try {
      await genererCertificatScolarite(
        etudiant.id,
        `certificat_scolarite_${etudiant.matricule}.pdf`,
      );
      toast.success("Certificat de scolarité généré.");
    } catch (err) {
      const data = err.response?.data;
      toast.error(
        data?.detail ?? "Erreur lors de la génération du certificat.",
      );
    }
  };
  ...
  return (
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[85vh] overflow-y-auto max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            Dossier — {etudiant.prenom} {etudiant.nom}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
          <div className="flex flex-wrap gap-2">
            ...
            >
              <FileCheck className="w-4 h-4" /> Générer un certificat de
              scolarité
            </Button>
          </div>
          {qrUrl && (
            ...
            {historique.length === 0 ? (
              ...
              <div className="space-y-1">
                {historique.map((i) => (
                  <div
                    key={i.id}
                    className="flex justify-between text-sm bg-muted rounded-lg px-3 py-2"
                  >
          ...
          <div>
            ...
            >
              ...
              <Button
                type="submit"
                size="sm"
                disabled={!fichier || createDocument.isPending}
                className="gap-2"
              >
            ...
            <div className="space-y-1">
              {mesDocuments.map((d) => (
                <div
                  key={d.id}
                  className="flex justify-between items-center text-sm bg-muted rounded-lg px-3 py-2"
                >
                  <a
                    href={d.fichier}
                    target="_blank"
