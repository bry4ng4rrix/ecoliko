...
export const demandeDocumentService = createResourceService(
  "/demandes-documents",
);
...
export const dossierEnseignantService = createResourceService(
  "/dossiers-enseignants",
);
export const paiementSalaireService =
  createResourceService("/paiements-salaire");
export const evenementCalendrierService = createResourceService(
  "/evenements-calendrier",
);
export const documentEtudiantService = createResourceService(
  "/documents-etudiants",
);
export const messageGroupeClasseService = createResourceService(
  "/messages-groupe-classe",
);
export const documentDevoirService =
  createResourceService("/documents-devoirs");
export const discussionClasseService = createResourceService(
  "/discussions-classe",
);

/** Ouvre ou ferme la discussion de groupe d'une classe (bascule réservée au professeur concerné). */
export async function definirDiscussionClasse(
  classeId,
  enseignantId,
  estOuverte,
) {
...
export async function synchroniserJoursFeries() {
  const { data } = await apiClient.post(
    "/evenements-calendrier/synchroniser-jours-feries/",
  );
  return data;
}
...
export async function envoyerRappelsDevoirs(joursAvant = 3) {
  const { data } = await apiClient.post("/cahier-textes/envoyer-rappels/", {
...
export async function genererBulletin({ etudiant, annee_scolaire, trimestre }) {
  const { data } = await apiClient.post("/bulletins/generer/", {
    etudiant,
    annee_scolaire,
    trimestre,
  });
...
export async function telechargerBulletinPdf(
  bulletinId,
...
export async function validerDemandeDocument(demandeId) {
  const { data } = await apiClient.post(
    `/demandes-documents/${demandeId}/valider/`,
  );
  return data;
}
...
export async function refuserDemandeDocument(demandeId, motif) {
  const { data } = await apiClient.post(
    `/demandes-documents/${demandeId}/refuser/`,
    { motif },
  );
  return data;
}
...
export async function fetchSyntheseFinanciere(anneeScolaireId) {
  ...
  const { data } = await apiClient.get("/paiements/synthese/", {
    params: { annee_scolaire: anneeScolaireId },
  });
  return data;
}
...
export async function fetchMoyenneTrimestre(etudiantId, trimestreId) {
  ...
  const { data } = await apiClient.get("/notes/moyenne/", {
    params: { etudiant: etudiantId, trimestre: trimestreId },
  });
  return data.moyenne;
}
...
export async function fetchClassement(classeId, trimestreId) {
  ...
  const { data } = await apiClient.get(`/classes/${classeId}/classement/`, {
    params: { trimestre: trimestreId },
  });
  return data;
}
...
export async function fetchClassementAnnuel(classeId) {
  ...
  const { data } = await apiClient.get(
    `/classes/${classeId}/classement-annuel/`,
  );
  return data;
}
...
export async function fetchStatistiques(anneeScolaireId, trimestreId) {
  ...
  const { data } = await apiClient.get("/statistiques/", {
    params: {
      annee_scolaire: anneeScolaireId,
      trimestre: trimestreId || undefined,
    },
  });
  return data;
}
...
export const fetchEtudiantQrCodeUrl = (etudiantId) =>
  fetchImageObjectUrl(`/etudiants/${etudiantId}/qrcode/`);
export const fetchEtudiantCodeBarreUrl = (etudiantId) =>
  fetchImageObjectUrl(`/etudiants/${etudiantId}/codebarre/`);

/** Télécharge la carte d'étudiant PDF (photo, matricule, QR code). */
export async function telechargerCarteEtudiant(
  etudiantId,
  nomFichier = "carte_etudiant.pdf",
) {
...
export async function genererCertificatScolarite(
  etudiantId,
  nomFichier = "certificat_scolarite.pdf",
) {
  return telechargerPdfPost(
    `/etudiants/${etudiantId}/certificat-scolarite/`,
    nomFichier,
  );
}

/** Télécharge la carte d'écolage PDF (reçu récapitulatif des paiements pour l'année active). */
export async function telechargerCarteEcolage(
  etudiantId,
  nomFichier = "carte_ecolage.pdf",
) {
  return telechargerPdf(`/etudiants/${etudiantId}/carte-ecolage/`, nomFichier);
}

/** Télécharge une facture d'écolage PDF pour un mois impayé ou le droit d'inscription. */
export async function telechargerFactureEcolage(
  etudiantId,
  { anneeScolaireId, mois, inscription = false, allowPaid = false },
  nomFichier,
) {
  const params = { annee_scolaire: anneeScolaireId };
  if (inscription) {
    params.type = "inscription";
  } else {
    params.mois = mois;
  }
  if (allowPaid) params.allow_paye = 1;
  const response = await apiClient.get(
    `/etudiants/${etudiantId}/facture-ecolage/`,
    {
      params,
      responseType: "blob",
    },
...
export async function soumettreJustification(presenceId, justificatif) {
  const { data } = await apiClient.post(`/presences/${presenceId}/justifier/`, {
    justificatif,
  });
  return data;
}
...
export async function validerJustification(presenceId) {
  const { data } = await apiClient.post(
    `/presences/${presenceId}/valider-justification/`,
  );
  return data;
}
...
export async function refuserJustification(presenceId) {
  const { data } = await apiClient.post(
    `/presences/${presenceId}/refuser-justification/`,
  );
  return data;
}
...
export async function activerAnneeScolaire(anneeScolaireId) {
  const { data } = await apiClient.post(
    `/annees-scolaires/${anneeScolaireId}/activer/`,
  );
  return data;
}

export const demandeInscriptionService = createResourceService(
  "/demandes-inscription",
);
export const pieceJointeInscriptionService = createResourceService(
  "/pieces-jointes-inscription",
);
